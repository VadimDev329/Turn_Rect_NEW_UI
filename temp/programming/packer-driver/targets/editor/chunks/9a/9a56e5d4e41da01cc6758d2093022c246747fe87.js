    ...
    execute: function () {
      ...
      _export("Game", Game = (_dec = ccclass('Game'), _dec2 = property({
        ...
        onTouchStart(event) {
          ...
          const target = event.target; // --- ЛОГИКА ШТРАФА (Усложнение) ---
        ...
        triggerPenaltyVisual() {
          ...
          const label = node.getComponent(Label); // 1. Сохраняем исходные данные, чтобы вернуться к ним

          const startPos = node.position.clone();
          const originalColor = label.color.clone(); // 2. Тряска относительно СТАРТОВОЙ позиции
...
//# sourceMappingURL=9a56e5d4e41da01cc6758d2093022c246747fe87.js.map